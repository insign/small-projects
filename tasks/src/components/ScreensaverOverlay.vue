<template>
  <div
    v-if="isActive"
    class="fullscreen transparent-overlay"
    @click="deactivate"
    @touchstart.prevent="deactivate"
  >
...
const generateConfetti = () => {
  ...
  // Remove old confetti pieces after 15 seconds to allow them to finish falling
  setTimeout(() => {
    confettiPieces.value = confettiPieces.value.filter(
      (c) => !newConfetti.find((nc) => nc.id === c.id),
    );
  }, 15000);
};
