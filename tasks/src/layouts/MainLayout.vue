  ...
  <q-layout view="lHh Lpr lFf">
    <q-header>
      <q-toolbar :class="`bg-${screensaverStore.toolbarColor}`">
        ...
        <q-btn flat dense round icon="sync" @click="() => sync(true)" :aria-label="t('buttons.sync')" :color="screensaverStore.buttonColor" />
        <q-btn flat dense round icon="add" @click="() => openTaskDialog(null)" :aria-label="t('buttons.addTask')" :color="screensaverStore.buttonColor" />
        <q-btn flat dense round icon="event_repeat" @click="openRecurringDialog"
          :aria-label="t('buttons.recurringTasks')" :color="screensaverStore.buttonColor">
          <q-badge v-if="recurringTaskCount > 0" color="red" floating>{{ recurringTaskCount }}</q-badge>
        </q-btn>
        <q-btn flat dense round icon="settings" @click="openSettingsDialog" :aria-label="t('buttons.settings')" :color="screensaverStore.buttonColor" />
        <q-btn flat dense round :icon="$q.fullscreen.isActive ? 'fullscreen_exit' : 'fullscreen'"
          @click="$q.fullscreen.toggle()" :aria-label="t('buttons.toggleFullscreen')" :color="screensaverStore.buttonColor" />
      </q-toolbar>
    </q-header>
